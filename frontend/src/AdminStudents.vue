<template>
  <AdminLayout activeRoute="students">
    <div class="student-list">
      <h2 class="page-title">考生数据</h2>

      <div class="stat-grid">
        <StatCard label="总考生" :value="data.total_students" color="blue" />
        <StatCard label="已通过" :value="data.passed_students" color="green" />
        <StatCard label="未通过" :value="data.failed_students" color="red" />
      </div>
        ...
        <table class="table" v-if="data.students?.length">
          ...
          <tbody>
            <tr v-for="s in data.students" :key="s.id">
              ...
              <td class="td-center">
                <router-link :to="`/admin/students/${s.id}`" class="btn-detail">详情</router-link>
              </td>
            </tr>
          </tbody>
        </table>
